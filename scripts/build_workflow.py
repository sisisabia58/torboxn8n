    ...
    nodes = [
        ...
        node("Account OK?", "n8n-nodes-base.code", 2, [440, 20], {
            ...
            "jsCode":
                ...
                "const now = Date.now();\n"
                "\n"
                "// cooldown_until is NOT a reliable block signal and must not gate\n"
                "// the run. A trial account had it set and every upload failed; a\n"
                "// paid account has it set just as far in the future and uploads\n"
                "// fine. It appears to be a rolling field rather than an\n"
                "// enforcement flag, so blocking on it rejects healthy accounts.\n"
                "// Carried through as information only.\n"
                "const cooldown = d.cooldown_until ? Date.parse(d.cooldown_until) : 0;\n"
                "const cooldownHrs = cooldown > now\n"
                "  ? ((cooldown - now) / 3600000).toFixed(1) : null;\n"
                "\n"
        ...
        node("Classify Failure", "n8n-nodes-base.code", 2, [3740, 620], {
            ...
            "jsCode":
                ...
                "  stage = 'token';\n"
                "} else if (/\\b401\\b|not authenticated|unauthori[sz]ed/i.test(raw)) {\n"
                "  stage = 'torbox-auth';\n"
                "  reason = 'TorBox rejected the API key (401). Check the "
                "\\u2018TorBox Bearer\\u2019 credential in n8n \\u2014 its value must be "
                "the word Bearer, a space, then the key. Pasting the bare key "
                "produces exactly this error. Original: ' + reason;\n"
                "} else if (/\\b403\\b|forbidden/i.test(raw)) {\n"
                "  stage = 'torbox-forbidden';\n"
